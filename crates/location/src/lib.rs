...
pub fn open_rto_db(path: &Utf8Path) -> anyhow::Result<Connection> {
...
pub fn set_watermark(db: &Connection, ms: i64) {
    db.execute(
...
/// Sweeps clipped Duet and subnet period arrays to find overlapping pairs and accumulates
/// their overlap duration as edge weights.
pub fn update_co_occurrences(
    ...
    // Only process duet periods with new data since the watermark.
    // For open-ended (last anchor) periods, clamp start to since_ms.
    ...
    for dp in &fresh_duet {
        // Advance subnet pointer past periods ending before this duet period starts
    ...
    db.execute_batch("BEGIN").expect("BEGIN failed");
    for pair in &pairs {
        db.execute(
        ...
        .expect("upsert co_occurrences failed");
    }
    db.execute_batch("COMMIT").expect("COMMIT failed");
}
...
/// Applies exponential decay to edge weights and runs Kruskal union-find to cluster signals
/// into location identities. Returns a map from signal to its cluster representative.
pub fn resolve_location_clusters(
    ...
    let mut edges: Vec<(f64, EdgeRow)> = stmt
        ...
        .map(|r| {
            let e = r.expect("row fetch failed");
    ...
    // Sort descending by effective weight
    edges.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));

    // Union-Find with duet-preferring root selection
    ...
    // Kruskal: merge edges above threshold in descending weight order
    ...
    // Build result: every known signal -> its cluster representative
...
/// Returns the cluster representative with the highest total decayed weight across all edges.
pub fn dominant_cluster(
    ...
    let edges: Vec<EdgeRow> = stmt
        ...
        .expect("query_map failed")
        .map(|r| r.expect("row fetch failed"))
        .collect();
...
/// Reads Duet anchor occurrences and returns them as step-function `SignalPeriod` slices.
pub fn collect_duet_periods(duet_db_path: Option<&Utf8Path>) -> Vec<SignalPeriod> {
    ...
    let anchors: Vec<AnchorRow> =
        match stmt.query_map(rusqlite::params![HOME_LOCATION_TYPE], |row| {
            Ok(AnchorRow {
                anchor_date: row.get(0)?,
                anchor_location: row.get(1)?,
            })
        }) {
...
/// Reads wifi IP events and returns them as step-function `SignalPeriod` slices.
pub fn collect_subnet_periods(wifi_log_db: &Connection) -> Vec<SignalPeriod> {
    ...
    let mut stmt = wifi_log_db
        .prepare("SELECT timestamp, subnet FROM ip_events ORDER BY timestamp")
        .expect("prepare ip_events failed");

    let rows: Vec<IpRow> = stmt
        .query_map([], |row| {
            Ok(IpRow {
                timestamp: row.get(0)?,
                subnet: row.get(1)?,
...
// ---------------------------------------------------------------------------
// Clip duet periods to office subnet windows (local duplicate, no circular dep)
// ---------------------------------------------------------------------------
...
pub fn load_rto_data(
    ...
    // Step 1: Schema migration v1 — prefix raw UUIDs with "duet:"
    ...
    if schema_version < RTO_SCHEMA_VERSION {
        rto_db.execute_batch("BEGIN").expect("BEGIN failed");
        rto_db
            .execute(
            ...
            .expect("migration UPDATE failed");
        rto_db
            .execute(
            ...
            .expect("schema_meta upsert failed");
        rto_db.execute_batch("COMMIT").expect("COMMIT failed");
    }

    // Step 2: Office subnets from wifi_log_db
    let mut ni_stmt = wifi_log_db
        .prepare("SELECT DISTINCT subnet FROM network_identities WHERE ssid = ?1")
        .expect("prepare network_identities failed");
    let office_subnets: std::collections::HashSet<String> = ni_stmt
        .query_map(rusqlite::params![office_ssid], |row| row.get(0))
        .expect("query network_identities failed")
        .filter_map(|r| r.ok())
        .collect();

    // Step 3: Build office subnet periods
    ...
    // Step 4: Collect and clip duet periods
    ...
    // Step 6: Resolve clusters and dominant
    ...
    // Step 7: Query evidence for requested dates
    let blocks: Vec<RtoBlock> = if dates.is_empty() {
        ...
        let mut stmt = rto_db.prepare(&sql).expect("prepare evidence failed");
...
mod tests {
    use super::{
        CLUSTER_THRESHOLD_SECONDS, HALF_LIFE_DAYS, cluster_at, dominant_cluster, order_signals,
        resolve_location_clusters,
    };
    use chrono::Utc;
    use rusqlite::Connection;
    use std::collections::HashMap;
    ...
    #[test]
    fn order_signals_duet_before_subnet() {
    ...
    #[test]
    fn order_signals_lex_within_same_prefix_duet() {
    ...
    #[test]
    fn order_signals_lex_within_same_prefix_subnet() {
    ...
    #[test]
    fn order_signals_already_sorted_duet_first() {
    ...
    fn cluster_at_unknown_signal() {
        ...
        assert_eq!(
            cluster_at(&clusters, "subnet:10.0.0.0/24"),
            "subnet:10.0.0.0/24"
        );
    }
    ...
    fn resolve_location_clusters_merges_above_threshold() {
        ...
        // Insert one edge with weight well above threshold (10000 seconds, recent)
        db.execute(
            "INSERT INTO co_occurrences (signal_a, signal_b, total_seconds, last_seen)
             VALUES (?1, ?2, ?3, ?4)",
        ...
        let clusters = resolve_location_clusters(&db, HALF_LIFE_DAYS, CLUSTER_THRESHOLD_SECONDS);

        // Both signals should map to the duet: representative
        assert_eq!(
            clusters.get("duet:aaa").map(|s| s.as_str()),
            Some("duet:aaa")
        );
        assert_eq!(
    ...
    fn resolve_location_clusters_no_merge_below_threshold() {
        let db = make_in_memory_rto_db();
        // Edge with weight below threshold (1 second, very old — effectively 0 after decay)
        let old_ms = 1_000_000i64; // long ago
        db.execute(
            "INSERT INTO co_occurrences (signal_a, signal_b, total_seconds, last_seen)
             VALUES (?1, ?2, ?3, ?4)",
        ...
        // Should remain separate (or empty since weight < threshold means no union was done)
        // Each signal maps to itself
        let rep_duet = clusters
            .get("duet:aaa")
            .map(|s| s.as_str())
            .unwrap_or("duet:aaa");
        let rep_subnet = clusters
            .get("subnet:10.0.0.0/24")
            .map(|s| s.as_str())
            .unwrap_or("subnet:10.0.0.0/24");
    ...
    fn dominant_cluster_basic() {
        ...
        let clusters = resolve_location_clusters(&db, HALF_LIFE_DAYS, CLUSTER_THRESHOLD_SECONDS);
        let dominant = dominant_cluster(&db, &clusters, HALF_LIFE_DAYS);

        // office-a has much higher weight
